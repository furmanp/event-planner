...
export async function createProjectsHandler(
  ...
  try {
    const result: Project | Prisma.BatchPayload = await createProject(
      projectData,
    );
...
export async function deleteProjectsHandler(
  _req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function getProjectByIdHandler(
  ...
  try {
    if (req.params.id) {
      const project: Project | null = await getProjectById(
